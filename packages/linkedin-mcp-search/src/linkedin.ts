...
function buildSearchUrl(params: JobSearchParams): string {
  ...
  if (params.experienceLevel?.length) {
    sp.set(
      'f_E',
      params.experienceLevel.map(e => EXPERIENCE_CODES[e]).join(','),
    );
  }
  ...
  if (params.start && params.start > 0) {
    const pageNum = Math.floor(params.start / 25);
    sp.set('pageNum', pageNum.toString());
...
function parseJobCard(
  ...
  if (!jobId) {
    jobId = extractJobId(
      $card.find('a.base-card__full-link, a').first().attr('href') || '',
    );
  }
  ...
  const salary =
    $card.find('.job-search-card__salary-info').text().trim() || undefined;
...
function parseJobDetails(html: string, jobId: string): JobDetails | null {
  ...
  try {
    ...
    const isEasyApply = $('button.jobs-apply-button')
      .text()
      .toLowerCase()
      .includes('easy apply');
...
function parseCompany(html: string, companyId: string): LinkedInCompany | null {
  ...
  try {
    ...
    const logo =
      $('img.org-top-card-primary-content__logo').attr('src') || undefined;
    const industry = $('div.org-top-card-summary-info-list__info-item')
      .first()
      .text()
      .trim();
    const website =
      $('a.org-top-card-primary-actions__action').attr('href') || undefined;
